...
def test_value_in_source_points(source_map, sources_field, mesh):
    test_result = True
    for source_cell in source_map.get_global_source_cells():
        source_cell_position = source_cell.get_position()
        if fu.is_point_inside_mesh(mesh, source_cell_position):
...
def test_value_near_source_points(source_map, sources_field, mesh, parameters):
    test_result = True
    ...
    for source_cell in source_map.get_global_source_cells():
        ...
        for test_position in test_positions:
            if fu.is_point_inside_mesh(mesh, test_position):
